    ...
    for img_path in img_paths:
        ...
        for batch in dataloader: 
            ...
            for n in range(batch_size):
                ...
                save_dir = os.path.join(args.out_folder, "predictions")
                os.makedirs(save_dir, exist_ok=True)

                # Save per-frame/per-hand outputs
                np.save(
                    os.path.join(save_dir, f"{img_fn}_{n}_verts.npy"),
                    verts
                )

                np.save(
                    os.path.join(save_dir, f"{img_fn}_{n}_joints3d.npy"),
                    joints
                )

                np.save(
                    os.path.join(save_dir, f"{img_fn}_{n}_kpts2d.npy"),
                    kpts_2d.cpu().numpy() if torch.is_tensor(kpts_2d) else kpts_2d
                )

                np.save(
                    os.path.join(save_dir, f"{img_fn}_{n}_cam_t.npy"),
                    cam_t
                )

                np.save(
                    os.path.join(save_dir, f"{img_fn}_{n}_is_right.npy"),
                    np.array(is_right)
                )

                # detector confidence
                det_score = detections[n].boxes.conf.cpu().numpy()

                np.save(
                    os.path.join(save_dir, f"{img_fn}_{n}_score.npy"),
                    det_score
                )
